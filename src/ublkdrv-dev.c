...
static inline void __ublkdrv_req_cells_free(struct ublkdrv_req const *req,
					    struct ublkdrv_ctx *ctx)
{
	switch (ublkdrv_cmd_get_op(&req->cmd)) {
	case UBLKDRV_CMD_OP_READ:
		__ublkdrv_sema_cells_free(
...
static void ublkdrv_req_cfq_push_work_h(struct work_struct *work)
{
	...
	struct uio_info *uinfo =
		&ubd->uios[UBLKDRV_UIO_DIR_KERNEL_TO_USER]->uio;
...
retry:
	...
	for (; ku_gate &&
	       !ublkdrv_cfq_push(ku_gate->cmdb->cmds, ku_gate->cmdb->cmds_len,
				 &ku_gate->cellc->cmdb_head,
				 &ku_gate->cmdb->tail, &req->cmd);
	     ku_gate = rcu_dereference(ubd->ku_gate)) {
		rcu_read_unlock();
		schedule();
		rcu_read_lock();
...
void ublkdrv_req_copy_work_h(struct work_struct *work)
{
	...
	switch (ublkdrv_cmd_get_op(&req->cmd)) {
	case UBLKDRV_CMD_OP_READ:
		ublkdrv_req_from_cells_to_bio_copy(
			cellc, bio, ctx->cells,
			ublkdrv_cmd_read_get_fcdn(&req->cmd.u.r));
		nwh = ublkdrv_req_finish_work_h;
		nwq = ubd->wqs[UBLKDRV_FIN_WQ];
		break;
	case UBLKDRV_CMD_OP_WRITE:
		ublkdrv_req_from_bio_to_cells_copy(
			cellc, ctx->cells, bio,
			ublkdrv_cmd_write_get_fcdn(&req->cmd.u.w));
		fallthrough;
	default:
...
static inline u32
ublkdrv_cell_gr_index_get(struct ublkdrv_cells_groups_ctx *ctx, u32 pages_nr)
{
	return ublkdrv_order_rounddown_and_clamp(
		pages_nr, 0, ARRAY_SIZE(ctx->cells_groups_state) - 1);
}

static int ublkdrv_dev_req_cells_acquire(struct ublkdrv_dev *ubd,
					 struct ublkdrv_ku_gate *ku_gate,
					 unsigned int sz,
					 struct ublkdrv_req *req)
{
	...
	for (struct ublkdrv_celld *prev_celld = &dummy_celld, *celld = NULL;
		...
		u32 const cell_gr_index_min_required =
			ublkdrv_cell_gr_index_get(cells_groups_ctx, pages_nr);

		cell_gr_index =
			min_t(int, cell_gr_index, cell_gr_index_min_required);

		for (; !(cell_gr_index < 0); --cell_gr_index) {
			celldn = dynamic_bitmap_semaphore_trywait(
				cells_groups_ctx
					->cells_groups_state[cell_gr_index]);
			if (likely(!(celldn < 0) &&
		...
		if (unlikely(cell_gr_index < 0)) {
			ublkdrv_sema_cells_free(ctx, dummy_celld.ncelld,
						cells_nr);
		...
		celld->data_sz =
			min_t(u32, sz, UBLKDRV_CELL_SZ_MIN << cell_gr_index);
...
static void ublkdrv_req_submit_work_h(struct work_struct *work)
{
	...
	if (unlikely(rc)) {
		ublkdrv_req_endio(req, rc < 0 ? errno_to_blk_status(rc) :
						BLK_STS_OK);
		return;
	}
...
{
	ublkdrv_req_submit(req, req->ubd->wqs[UBLKDRV_SUBM_WQ],
			   ublkdrv_req_submit_work_h);
}
