...
static void ublkdrv_submit_bio_fit(struct bio *bio, unsigned long start_j,
				   struct ublkdrv_dev *ubd)
{
	struct ublkdrv_req *req;

	req = kmem_cache_alloc_node(ubd->req_kc, GFP_KERNEL | __GFP_ZERO,
				    ubd->nid);
	if (unlikely(!req)) {
...
static int ublkdrv_ctx_init(struct ublkdrv_ctx *ctx, int nid)
{
	...
	cmds_len = (PAGE_ALIGN(sizeof(*cmdb)) - sizeof(*cmdb)) /
		   sizeof(cmdb->cmds[0]);
	cmds_len = clamp_t(u32, cmds_len, UBLKDRV_CMDS_LEN_MIN,
			   UBLKDRV_CMDS_LEN_MAX);
	...
	sz = UBLKDRV_CTX_CELLS_PER_GROUP * UBLKDRV_CELL_SZ_MIN *
	     ((1u << UBLKDRV_CTX_CELLS_GROUPS_NR) - 1);
	cells = vzalloc(sz);
	if (!cells)
		goto free_cmdb;
	...
	for (cell_off = 0, celldn = 0; celldn < ctx->cellc->cellds_len;
	     ++celldn) {
		...
		cell_off += UBLKDRV_CELL_SZ_MIN
			    << (celldn / UBLKDRV_CTX_CELLS_PER_GROUP);
	}
	...
	ctx->ku_state_ctx =
		kzalloc_node(sizeof(*ctx->ku_state_ctx), GFP_KERNEL, nid);
	if (!ctx->ku_state_ctx) {
	...
	ctx->ku_state_ctx->cmds_ids = kzalloc_node(
		sizeof(*ctx->ku_state_ctx->cmds_ids), GFP_KERNEL, nid);
	if (!ctx->ku_state_ctx->cmds_ids) {
		pr_err("unable to allocate command IDs semaphore bitmap, out of memory\n");
		goto free_ku_state_ctx;
	}

	r = dynamic_bitmap_semaphore_init(ctx->ku_state_ctx->cmds_ids,
					  cmdb->cmds_len - 1, nid);
	if (r) {
	...
	ctx->ku_state_ctx->reqs_pending = kcalloc_node(
		cmdb->cmds_len - 1, sizeof(ctx->ku_state_ctx->reqs_pending[0]),
		GFP_KERNEL, nid);
	...
	ctx->cells_groups_ctx =
		kzalloc_node(sizeof(*ctx->cells_groups_ctx), GFP_KERNEL, nid);
	...
	for (i = 0; i < ARRAY_SIZE(ctx->cells_groups_ctx->cells_groups_state);
	     ++i) {
		struct dynamic_bitmap_semaphore *cells_group_state =
			kzalloc_node(sizeof(*cells_group_state), GFP_KERNEL,
				     nid);
		...
		r = dynamic_bitmap_semaphore_init(
			cells_group_state, UBLKDRV_CTX_CELLS_PER_GROUP, nid);
		if (r) {
		...
		ctx->cells_groups_ctx->cells_groups_state[i] =
			cells_group_state;
	}
...
destroy_cells_groups_state:
	for (j = i - 1; !(j < 0); --j) {
		struct dynamic_bitmap_semaphore *cells_group_state =
			ctx->cells_groups_ctx->cells_groups_state[j];
...
static void ublkdrv_ctx_deinit(struct ublkdrv_ctx *ctx)
{
	...
	for (i = ARRAY_SIZE(ctx->cells_groups_ctx->cells_groups_state) - 1;
	     !(i < 0); --i) {
		struct dynamic_bitmap_semaphore *cells_groups_state =
			ctx->cells_groups_ctx->cells_groups_state[i];
				       ...
				       int nid)
{
	...
	ubd->wqs[UBLKDRV_FIN_WQ] =
		alloc_workqueue("kreqack/%s", WQ_MEM_RECLAIM, 1, gd->disk_name);
	...
	ubd->wqs[UBLKDRV_CFQ_POP_WQ] =
		alloc_workqueue("kcmdack/%s", WQ_UNBOUND, 1, gd->disk_name);
	if (!ubd->wqs[UBLKDRV_CFQ_POP_WQ]) {
		pr_err("unable to allocate a workqueue for cfq commands popping, out of memory\n");
		goto destroy_wqs;
	}

	ubd->wqs[UBLKDRV_CFQ_PUSH_WQ] =
		alloc_workqueue("kcmdsubm/%s", WQ_UNBOUND, 1, gd->disk_name);
...
static int __init ublkdrv_init(void)
{
	...
	pr_info("%s-%s init for kernel %s %s", module_name(THIS_MODULE),
		THIS_MODULE ? THIS_MODULE->version : "unknown",
