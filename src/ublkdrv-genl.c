...
static const struct nla_policy nl_policy[UBLKDRV_GENL_BDEV_ATTRS_QTY] = {
    [UBLKDRV_GENL_BDEV_ATTR_NAME_SUFFIX]      = {.type = NLA_STRING, .len = DISK_NAME_LEN - strlen(UBLKDRV_PREFIX)},
...
static int ublkdrv_genl_bdev_cmd_create(struct sk_buff* skb, struct genl_info* info);
static int ublkdrv_genl_bdev_cmd_destroy(struct sk_buff* skb, struct genl_info* info);
...
static int ublkdrv_genl_bdev_cmd_create(struct sk_buff* skb, struct genl_info* info)
{
    ...
    if (info->attrs[UBLKDRV_GENL_BDEV_ATTR_READ_ONLY])
        read_only = nla_get_flag(info->attrs[UBLKDRV_GENL_BDEV_ATTR_READ_ONLY]);

    nla_strscpy(disk_name + strlen(UBLKDRV_PREFIX), info->attrs[UBLKDRV_GENL_BDEV_ATTR_NAME_SUFFIX], DISK_NAME_LEN - strlen(UBLKDRV_PREFIX));
    capacity = nla_get_u64(info->attrs[UBLKDRV_GENL_BDEV_ATTR_CAPACITY_SECTORS]);

    devices[devs_nr] = ublkdrv_dev_create(disk_name, capacity, read_only, nid);
    if (IS_ERR_OR_NULL(devices[devs_nr]))
        return devices[devs_nr] ? PTR_ERR(devices[devs_nr]) : -ENOMEM;

    pr_info("bdev %s (disk %s) registered\n", devices[devs_nr]->name, devices[devs_nr]->disk->disk_name);
...
static int ublkdrv_genl_bdev_cmd_destroy(struct sk_buff* skb, struct genl_info* info)
{
    ...
    for (i = 0; i < devs_nr; ++i) {
        ...
        if (!strncmp(ubd->disk->disk_name, disk_name, sizeof(disk_name))) {
            pr_info("unregister bdev %s (disk %s)\n", ubd->name, ubd->disk->disk_name);
