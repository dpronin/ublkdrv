...
static void ublkdrv_cfq_pop_work_h(struct work_struct *work)
{
	struct ublkdrv_cmd_complete_work *ccw =
		container_of(work, struct ublkdrv_cmd_complete_work, work);
...
retry:
		...
		if (unlikely(!ublkdrv_cfq_ack_pop(cmdb_ack->cmds,
						  cmdb_ack->cmds_len,
						  &cellc->cmdb_ack_head,
						  &cmdb_ack->tail, &cmd_ack))) {
			rcu_read_unlock();
			schedule();
			goto retry;
		}
		...
		if (likely(0 == dynamic_bitmap_semaphore_post(
					ctx->ku_state_ctx->cmds_ids, cmd_id))) {
...
static int ublkdrv_uio_irq_cmd_handled(struct uio_info *info, s32 irq_on)
{
	...
	if (likely(irq_on)) {
		...
		do {
			ph_old = ph;
			cpu_relax();
		} while (ph_old != (ph = cmpxchg_relaxed(
					    &cellc->cmdb_head, ph,
					    (ph + irq_on) % cmdb->cmds_len)));
	}
...
static void ublkdrv_uio_vma_close(struct vm_area_struct *vma)
{
	...
	if (test_and_clear_bit(mem_id, &uio->flags) &&
	    !smp_load_acquire(
		    &ubd->uios[UBLKDRV_UIO_DIR_KERNEL_TO_USER]->flags) &&
	    !smp_load_acquire(
		    &ubd->uios[UBLKDRV_UIO_DIR_USER_TO_KERNEL]->flags)) {
		int i;
		...
		ctx->cmdb->tail = 0;
		memset(ctx->cmdb->cmds, 0,
		       sizeof(ctx->cmdb->cmds[0]) * ctx->cmdb->cmds_len);
		...
		ctx->cmdb_ack->tail = 0;
		memset(ctx->cmdb_ack->cmds, 0,
		       sizeof(ctx->cmdb_ack->cmds[0]) *
			       ctx->cmdb_ack->cmds_len);
		ctx->cellc->cmdb_ack_head = 0;

		for (i = 0; i < ctx->cmdb->cmds_len - 1; ++i) {
			struct ublkdrv_req *req =
				ctx->ku_state_ctx->reqs_pending[i];
			if (req) {
				BUG_ON(dynamic_bitmap_semaphore_post(
					ctx->ku_state_ctx->cmds_ids, i));
...
static vm_fault_t ublkdrv_uio_vma_fault(struct vm_fault *vmf)
{
	...
	page = uio->uio.mem[mem_id].memtype == UIO_MEM_LOGICAL ?
		       virt_to_page(addr) :
...
static int ublkdrv_uio_mmap(struct uio_info *info, struct vm_area_struct *vma)
{
	...
	if (uio == ubd->uios[UBLKDRV_UIO_DIR_KERNEL_TO_USER] &&
		...
		struct ublkdrv_ku_gate *ku_gate =
			kzalloc_node(sizeof(*ku_gate), GFP_KERNEL, ubd->nid);
		BUG_ON(!ku_gate);
	...
	if (uio == ubd->uios[UBLKDRV_UIO_DIR_USER_TO_KERNEL] &&
		...
		struct ublkdrv_uk_gate *uk_gate =
			kzalloc_node(sizeof(*uk_gate), GFP_KERNEL, ubd->nid);
		BUG_ON(!uk_gate);
...
static int ublkdrv_uio_kern_to_user_register(struct ublkdrv_dev *ubd,
					     struct ublkdrv_uio *uio)
{
	...
	uio_name = kzalloc(
		strlen(ubd->name) +
			strlen(UBLKDRV_UIO_KERNEL_TO_USER_DIR_SUFFIX) + 1,
		GFP_KERNEL);
	if (!uio_name)
		goto err;

	strcat(strcpy(uio_name, ubd->name),
...
static int ublkdrv_uio_user_to_kernel_register(struct ublkdrv_dev *ubd,
					       struct ublkdrv_uio *uio)
{
	struct uio_info *info;
	char *uio_name;
	...
	uio_name = kzalloc(
		strlen(ubd->name) +
			strlen(UBLKDRV_UIO_USER_TO_KERNEL_DIR_SUFFIX) + 1,
		GFP_KERNEL);
	if (!uio_name)
		goto err;
...
int ublkdrv_uios_register(struct ublkdrv_dev *ubd)
{
	...
	static int (*f[])(struct ublkdrv_dev *, struct ublkdrv_uio *) = {
		[UBLKDRV_UIO_DIR_KERNEL_TO_USER] =
			ublkdrv_uio_kern_to_user_register,
		[UBLKDRV_UIO_DIR_USER_TO_KERNEL] =
			ublkdrv_uio_user_to_kernel_register,
	};
	...
	for (i = 0; i < ARRAY_SIZE(f); ++i) {
		ubd->uios[i] = kzalloc_node(sizeof(*ubd->uios[i]), GFP_KERNEL,
					    ubd->nid);
		if (!ubd->uios[i]) {
			ret = -ENOMEM;
			goto err;
