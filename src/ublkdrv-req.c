					...
					u32 celldn)
{
	...
	bio_for_each_segment(bv, bio, iter) {
		...
		for (cp_sz = min(data_sz, bv_len); bv_len; bv_offset += cp_sz,
		    bv_len -= cp_sz, cp_sz = min(data_sz, bv_len)) {
			char *to =
				&((char *)(cells))[celld->offset + cell_offset];
			char const *from =
				&((char const *)(bv_page))[bv_offset];
					...
					u32 celldn)
{
	...
	for (; celldn < cellc->cellds_len && !celld->data_sz;
	     celldn = celld->ncelld, celld = &cellc->cellds[celldn])
		;
