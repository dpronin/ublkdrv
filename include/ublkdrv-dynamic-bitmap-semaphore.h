...
static inline void
dynamic_bitmap_semaphore_destroy(struct dynamic_bitmap_semaphore *dbsem)
{
	kfree(dbsem->map);
	dbsem->map = NULL;
}
...
dynamic_bitmap_semaphore_weight(struct dynamic_bitmap_semaphore const *dbsem)
{
	return dbsem->set_bits;
}

static inline unsigned int dynamic_bitmap_semaphore_weight_max(
	struct dynamic_bitmap_semaphore const *dbsem)
{
	return dbsem->bits;
}

static inline int
dynamic_bitmap_semaphore_trywait(struct dynamic_bitmap_semaphore *dbsem)
{
	unsigned long bit;
	...
	bit = find_first_bit(dbsem->map,
			     dynamic_bitmap_semaphore_weight_max(dbsem));
...
static inline int
dynamic_bitmap_semaphore_post(struct dynamic_bitmap_semaphore *dbsem,
			      unsigned int pos)
{
